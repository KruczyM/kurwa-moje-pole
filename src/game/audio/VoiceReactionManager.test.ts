...
type FakeAudio={src:string;volume:number;currentTime:number;play:()=>Promise<void>;pause:()=>void};
const fakeAudio=():FakeAudio=>({src:'',volume:0,currentTime:0,play:vi.fn(async()=>undefined),pause:vi.fn()});
...
describe('VoiceReactionManager',()=>{
 ...
 it('chooses one sound from a pool and avoids immediate repetition',()=>{
  const channels=[fakeAudio(),fakeAudio()],manager=new VoiceReactionManager(()=>0,()=>channels.shift()!);
 ...
 it('prepares overdose audio after four uses in one minute',()=>{
  const foreground=fakeAudio(),distant=fakeAudio();
  const manager=new VoiceReactionManager(()=>0,(()=>{const channels=[foreground,distant];return()=>channels.shift()!})());
