...
export class NpcAnimator{
 ...
 constructor(root:THREE.Object3D,clips:THREE.AnimationClip[]){this.mixer=new THREE.AnimationMixer(root);for(const clip of clips){const canonical=resolveCanonicalAnimationName(clip.name);if(canonical)this.actions.set(canonical,this.mixer.clipAction(clip))}this.play('Idle',0)}
 ...
 play(name:LocomotionClip,fade=.2){
